...
extern "C" u32 GenerateRandomNumber(int max); // Originally from boss.hpp, moved here for convenience
...
namespace EGG {
	// idk if this is correct but whatever
	class Disposer {
		public:
			void *vtable;
...
// handles everything related to stockItem arrays, changing this can and will break stuff
// be sure to fix STOCK_ITEM and STOCK_ITEM_SHADOW class sizes when changing this!!!
#define POWERUP_COUNT 10
#define NEW_POWERUP_COUNT POWERUP_COUNT - 7

// actual number of powerups in the game, including ones not in stockItem
#define TRUE_POWERUP_COUNT 13 // ?
...
public:
	...
	// new
	daWMItem_c *newItemPtr[POWERUP_COUNT];
	int newCounts[POWERUP_COUNT];
	nw4r::lyt::Picture *newButtonBase[POWERUP_COUNT];
	nw4r::lyt::Pane *newIconPanes[POWERUP_COUNT];
	nw4r::lyt::Picture *newIconPictures[POWERUP_COUNT];
...
public:
	...
	int counts[POWERUP_COUNT]; // first 7 are part of original class

	// new
	nw4r::lyt::TextBox *newX[POWERUP_COUNT];
	nw4r::lyt::TextBox *newValue[POWERUP_COUNT];
	nw4r::lyt::Picture *newButtonBase[POWERUP_COUNT];
