...
dKPMapHud_c *dKPMapHud_c::instance = 0;

dActor_c *dKPMapHud_c::build() {
	void *buffer = AllocFromGameHeap1(sizeof(dKPMapHud_c));
	dKPMapHud_c *c = new(buffer) dKPMapHud_c;

	instance = c;
	return c;
}

dKPMapHud_c::dKPMapHud_c() {
	layoutLoaded = false;
...
int dKPMapHud_c::onCreate() {
	if (!layoutLoaded) {
		...
		int groupIDs[] = {0, 1, 0, 2, 3};

		layout.loadAnimations(brlanNames, 4);
		layout.loadGroups(groupNames, groupIDs, 5);
		...
		int msgIDs[2] = {12, 15};
		layout.setLangStrings(tbNames, msgIDs, 4, 2);
...
void dKPMapHud_c::loadInitially() {
	if (doneFirstShow)
		return;
...
int dKPMapHud_c::onDelete() {
	dKPMapHud_c::instance = 0;
...
void dKPMapHud_c::playShowAnim(int id) {
	if (!this || !this->layoutLoaded) return;

	layout.enableNonLoopAnim(id);
}

void dKPMapHud_c::playHideAnim(int id) {
	if (!this || !this->layoutLoaded) return;
...
void dKPMapHud_c::loadHeaderInfo() {
...
void dKPMapHud_c::loadFooterInfo() {
	SaveBlock *save = GetSaveFile()->GetBlock(-1);
	...
	dLevelInfo_c::entry_s *lastLevel = linfo->searchByDisplayNum(save->current_world, lastLevelIDs[save->current_world]);
	if (lastLevel) {
	...
	dLevelInfo_c::section_s *sect = linfo->getSectionByIndex(save->current_world);
...
void dKPMapHud_c::enteredNode(dKPNode_s *node) {
	if (node == 0)
...
void dKPMapHud_c::hideFooter() {
	if (isFooterVisible)
		playHideAnim(SHOW_FOOTER);
}

void dKPMapHud_c::showFooter() {
	if (!doneFirstShow)
		return;
	willShowFooter = true;
	if (isFooterVisible)
...
void dKPMapHud_c::updatePressableButtonThingies() {
	...
	if (cntType != displayedControllerType) {
		...
		dInfo_c::instance->currentControllerType = beef;
