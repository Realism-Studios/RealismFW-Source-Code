...
#include "koopatlas/kpWorldMap.hpp"
#include <sfx.hpp>
#include <stage.hpp>
...
void dWMPathManager_c::setup() {
	...
	if (LastPowerupStoreType == LOSE_LEVEL) {
		...
		daKPPlayer_c::instance->visible = false;
...
bool dWMPathManager_c::evaluateUnlockCondition(u8 *&in, SaveBlock *save, int stack) {
	...
	u8 subConditionType, one, two;
	
	if (conditionType == 0) {
		subConditionType = controlByte & 0x3F;
...
void dWMPathManager_c::execute() {
	...
	if (waitAtStart > 0) {
		...
		if (waitAtStart == 0) {
			if (mustPlayAfterDeathAnim) {
				daKPPlayer_c::instance->visible = true;
				daKPPlayer_c::instance->startAnimation(ending_wait, 1.0f, 0.0f, 0.0f);
				...
				if (dScKoopatlas_c::instance->isAfter8Castle) {
					...
					daKPPlayer_c::instance->startAnimation(dm_surp_wait, 1.0f, 0.0f, 0.0f);
	...
	if (waitAfterInitialPlayerAnim > 0) {
		...
		if (waitAfterInitialPlayerAnim == 0)
			daKPPlayer_c::instance->startAnimation(wait_select, 1.0f, 0.0f, 0.0f);
	...
	if (countdownToFadeIn > 0) {
		...
		if (countdownToFadeIn <= 0) {
			if (camBoundsValid) {
				dKPCamera_c::instance->currentX = currentNode->x;
				dKPCamera_c::instance->currentY = -currentNode->y;
				dKPCamera_c::instance->panToBounds(camMinX, camMinY, camMaxX, camMaxY);
	...
	if (panningCameraToPaths) {
		if (dKPCamera_c::instance->panning)
			return;
	...
	if (waitBeforePanBack > 0) {
		...
		if (waitBeforePanBack == 0 && camBoundsValid) {
			dKPCamera_c::instance->panToPosition(
	...
	if (panningCameraFromPaths) {
		...
		dKPCamera_c::instance->followPlayer = true;
	}
	...
	if (completionMessageType > 0) {
		...
		daKPPlayer_c::instance->startAnimation(coin_comp, 1.0f, 0.0f, 0.0f);
		dmGladDuration = 154;
	...
	if (!initialLoading) {
		...
		dKPMapHud_c::instance->loadInitially();
		initialLoading = true;
		return;
	}
	...
	if (isMoving) {
		...
		if (pressedDir >= 0) {
			...
			if (canUseExit(currentNode->exits[pressedDir])) {
				...
				daKPPlayer_c::instance->setTargetRotY(directions[pressedDir]);
			}
...
void dWMPathManager_c::startMovementTo(dKPPath_s *path) {
	...
	if (currentNode && dKPMapHud_c::instance)
		dKPMapHud_c::instance->leftNode();
	...
	daKPPlayer_c *player = daKPPlayer_c::instance;
...
void dWMPathManager_c::moveThroughPath(int pressedDir) {
	...
	Vec move = {to->x - from->x, to->y - from->y, 0};
	...
	if (to->type == dKPNode_s::LEVEL && !calledEnteredNode) {
		...
		if (distToEnd < 64.0f && dKPMapHud_c::instance) {
			calledEnteredNode = true;
			dKPMapHud_c::instance->enteredNode(to);
		}
	}
	...
	if (
		...
		if (to->type == dKPNode_s::WORLD_CHANGE) {
			...
			if (world) {
				...
				if (wzHack) {
					//save->hudHintH = 2000;
					dKPMapHud_c::instance->hideFooter();
				} else {
					if (visiblyChange && dKPMapHud_c::instance)
						dKPMapHud_c::instance->showFooter();
				}
		...
		if (to->type == dKPNode_s::CHANGE) {
			...
			if (!calledEnteredNode && dKPMapHud_c::instance)
				dKPMapHud_c::instance->enteredNode();
...
void dWMPathManager_c::activatePoint() {
	...
	if (currentNode->type == dKPNode_s::LEVEL) {
		...
		daKPPlayer_c::instance->startAnimation(course_in, 1.2, 10.0, 0.0);
		daKPPlayer_c::instance->setTargetRotY(0);
