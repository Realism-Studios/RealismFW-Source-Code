...
bool daMegaGoomba_c::collisionCat7_GroundPound(ActivePhysics *apThis, ActivePhysics *apOther) { 
	...
	killWithSpecifiedState(apOther->owner, &eSpeed, &dEn_c::StateID_DieOther);
	return true; 
	// TODO: convince kirblue to let me make a thing were he can't die when ground pounded
	// FUCK YOU WHOEVER ADDED THIS. IT'S A BOSS, NO?
}
...
void daMegaGoomba_c::setupBodyModel() {
	...
	nw4r::g3d::ResAnmTexPat texPatRes = this->resFile.GetResAnmTexPat("walk");
	anmTexPat.setup(mdl, texPatRes, &allocator, 0, 1);
	anmTexPat.bindEntry(&bodyModel, &texPatRes, 0, 0);
	bodyModel.bindAnim(&anmTexPat, 0.0);
