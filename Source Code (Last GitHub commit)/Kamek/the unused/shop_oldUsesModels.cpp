...
const char *ShopFileList[] = {"lakitu","I_kinoko","I_fireflower","I_propeller","I_iceflower","I_penguin","I_kinoko_bundle","I_star","I_hammer",0};
...
int dWMShop_c::onCreate() {
	StageC4::instance->_1D = 1; // Disable pausing
	WLClass::instance->demoControlAllPlayers(); // Disable player control

	// Spritedata
	itemSet = settings & 0xF0000000;  // Nybble 5
	lakituTex = settings & 0xF000000; // Nyb 6
	rndItems = settings & 0xF00000;   // Nyb 7
	cursorVis = settings & 0xF;		  // Nyb 12

	// Now put together our layout
	if (!layoutLoaded) {
	...
	if (itemSet == 0) {
		...
		WriteBMGToTextBox(BackText, GetBMG(), 2, 34, 0);
	}

	this->show(CurrentWorld+1);
...
void dWMShop_c::endState_HideWait() {
	deleteModels();
	visible = false;
	ExitStage(3, 0, EXIT_LEVEL, FADE_OUT); // Return to World Map (scene 3)
}
...
void dWMShop_c::loadModels() {
	...
	for (int i = 0; i < ITEM_COUNT; i++) {
		...
		itemModels[i].setupItem(effectiveX, effectiveY, Inventory[itemSet][i]);
	}
}
