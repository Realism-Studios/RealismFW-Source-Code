...
public:
	...
	// Stuff for the fake crash
	void doFakeCrashStuff();
	m2d::EmbedLayout_c layout;
	dPlayerInput_c playerControls;
...
int dLevelSpecial_c::onExecute() {
	...
	if (newEvState == ACTIVATE)
	{
		...
		switch (this->type) {
			...
			case 10:										// Fake-Crash the game
				showFakeCrash = true;
				//this->doFakeCrashStuff();
				break;
...
// Code for the "Acception handlers?" level
/*void dLevelSpecial_c::doFakeCrashStuff() {
	int nowPressed = Remocon_GetPressed(GetActiveRemocon());
	
	layout.loadArc("fakeCrash.arc", false);
	layout.build("fakeCrash.brlyt");
	
	layout.update();
	layout.scheduleForDrawing();
	
	N_crash_00 = layout.findPaneByName("N_crash_00");
	P_dolphinBG_00 = layout.findPictureByName("P_dolphinBG_00");

	if (showFakeCrash) {
		// Show everything
		//N_crash_00->alpha = 255;
		
		
		playerControls.deactivate();
		
		if (isUsingDolphinEmulator) {
			// In Dolphin, the space outside of the exception handler box is rendered as just solid pink,
			// whereas on Wii hardware, the last frame of the game prior to the crash is still visible.
			P_dolphinBG_00->SetVisible(true);
		}
	}
	
	if (!showFakeCrash) {
		// Hide everything when showFakeCrash is false
		N_crash_00->alpha = 0;
		P_dolphinBG_00->SetVisible(false); // Might as well just to be safe
	}
