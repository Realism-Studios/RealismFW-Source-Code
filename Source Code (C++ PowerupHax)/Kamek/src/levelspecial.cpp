...
void FuckinBubbles() {
	...
	for (int i = 0; i < course->zoneSpriteCount[zone]; i++) {
		dCourse_c::Sprite *spr = &course->zoneFirstSprite[zone][i];
...
void dLevelSpecial_c::sendToOtherLevel() {
	// Spritedata
	int destWorld = (settings & 0xF000) >> 12; // Nybble 9
	int destStage = (settings & 0xFF0000) >> 16; // Nybbles 7/8
	int destArea = (settings & 0xF00) >> 8; // Nybble 10
	//int destEntrance = (settings & 0xFF00000000) >> 32; // THIS DOESN'T WORK
	bool showPreGame = ((settings & 0x10) != 0); // Nybble 11, value 1

	// Setup info for destination level
	RESTART_CRSIN_LevelStartStruct.screenType = 0; // Force regular level behavior, rather than titlescreen
	RESTART_CRSIN_LevelStartStruct.world1 = destWorld - 1;
	RESTART_CRSIN_LevelStartStruct.world2 = destWorld - 1;
	RESTART_CRSIN_LevelStartStruct.level1 = destStage - 1;
	RESTART_CRSIN_LevelStartStruct.level2 = destStage - 1;
	RESTART_CRSIN_LevelStartStruct.area = destArea;
	RESTART_CRSIN_LevelStartStruct.entrance = 0xFF; // 0xFF goes to Entrance ID set in the Area settings
	DontShowPreGame = showPreGame;
	// Start the new level
	ExitStage(RESTART_CRSIN, 0, EXIT_LEVEL, WIPE_CIRCLE);
}
...
int dLevelSpecial_c::onExecute() {
	...
	void *frameBuffer;
	GXColorYUV textColor = {1, 0, 0, NULL};
	...
	if (newEvState == ACTIVATE)
	{
		...
		switch (this->type) {
			...
			case 11:										// Load Any Level
				sendToOtherLevel();
				break;

			case 12:										// Return to the Wii Menu
				OSReturnToMenu();
				break;
