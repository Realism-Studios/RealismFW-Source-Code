...
const SpriteData NipperSpriteData = {ProfileId::NipperPlant, 8, 0xFFFFFFF0, 0, 8, 8, 8, 0, 0, 0, 0, 0};
Profile NipperProfile(&daEnPuchiPakkun_c::build, SpriteId::NipperPlant, &NipperSpriteData, ProfileId::EN_BLACK_PAKKUN, ProfileId::NipperPlant, "NipperPlant", NipperArcList, 0x12);
...
void daEnPuchiPakkun_c::playChrAnim(const char* name, int playsOnce, float unk, float rate, bool synced) {
    ...
    if (synced) {
        ...
        float frame = Stage_exeFrame - (Stage_exeFrame / int(anmChr.numFrames)) * int(anmChr.numFrames);
...
void daEnPuchiPakkun_c::executeState_FireSpit() {
    ...
    if (fireTimer == 15 && isPlayerInFireRange()) {
        create(NipperPlantFire, fireDist << 4 | fireDirection, &(Vec){pos.x, pos.y + 10.0, pos.z}, &rot, currentLayerID);
