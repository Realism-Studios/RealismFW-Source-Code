...
#define NEW_STOCK_COUNT POWERUP_COUNT - 7
	...
	for (int i = 0; i < 4; i++) {
		if (isPlayerActive[i]) {
			...
			if (picID != 24) {
				...
				player2d[charID]->pos.x = out.x;
				player2d[charID]->pos.y = out.y;
				player2d[charID]->pos.z = out.z;
				player2d[charID]->someVec.x = 0.89999998f;
				player2d[charID]->someVec.y = 0.89999998f;
				player2d[charID]->someVec.z = 0.89999998f;
				player2d[charID]->_25C = 26.0f;
			}
			howManyPlayers++;
		}
	}
...
void dStockItem_c::initializeState_WindowOpenAnimeEndWait() {
	...
	for (int i = 1; i < 8; i++)
		layout.resetAnim(i);
	// and new anims
	for (int i = 0; i < POWERUP_COUNT-7; i++) {
		int anim = i * 3 + ON_HAMMER;
...
void dStockItem_c::initializeState_ButtonChangeAnimeEndWait() {
	...
	int onAnim = selectedItem + ON_KINOKO;
	if (selectedItem > 6)
		onAnim = (selectedItem - 7) * 3 + ON_HAMMER;
...
void dStockItem_c::initializeState_HitAnimeEndWait() {
	...
	int animID = selectedItem + HIT_KINOKO;
	if (selectedItem > 6)
		animID = (selectedItem - 7) * 3 + HIT_HAMMER;
	layout.enableNonLoopAnim(animID);
}
