...
class daShyGuy : public dEn_c {
	...
	DECLARE_STATE(Spike);
	DECLARE_STATE(Dirt);
	DECLARE_STATE(BlackBelt);
	DECLARE_STATE(Rocket);
...
///////////////////////
	...
	CREATE_STATE(daShyGuy, Dirt);
	CREATE_STATE(daShyGuy, BlackBelt);
	CREATE_STATE(daShyGuy, Rocket);
...
int daShyGuy::onCreate() {
	...
	if (type == 1 || type == 8) {
		...
		this->resFile.data = getResource("shyguy", "g3d/ShyGuyBrown.brres");
	}
	else if (type == 10) {
		this->resFile.data = getResource("shyguy", "g3d/ShyGuyBlack.brres");
	}
	else if (type == 11) {
		this->resFile.data = getResource("shyguy", "g3d/ShyGuyWhite.brres");
	}
	else {
	...
	if (type == 0) {
		...
		doStateChange(&StateID_BlackBelt);
	}
	else if (type == 11) {
		doStateChange(&StateID_Rocket);
	}
...
// Rocket State
///////////////
	void daShyGuy::beginState_Rocket() {
		this->max_speed.x = 0.0;
		this->speed.x = 0.0;
		this->x_speed_inc = 0.0;
	...
	void daShyGuy::executeState_Rocket() {
		...
		if (this->pos.y < this->Baseline) {
			...
			// Then Rocket!
				...
				if (this->jumpCounter == 5) { this->jumpCounter = 0; }
				...
				if (this->jumpCounter == 5) {
					bindAnimChr_and_setUpdateRate("c18_NORMAL_STEAL_R", 1, 0.0, 1.0);
					SpawnEffect("Wm_mr_hardhit", 0, &(Vec){this->pos.x + 7.0, this->pos.y + 14.0, this->pos.z - 5500.0}, &(S16Vec){0,0,0}, &(Vec){0.8, 0.8, 0.8});
					this->speed.y = 20.0;
		...
		// While he's rocketing, it's time for gravity.
		else {
...
// BlackBelt State
///////////////
	void daShyGuy::beginState_BlackBelt() {
		this->max_speed.x = 0.0;
		this->speed.x = 0.0;
		this->x_speed_inc = 0.0;
		this->pos.y -= 4.0;
	...
	void daShyGuy::executeState_BlackBelt() {
	...
	void daShyGuy::endState_BlackBelt() {
	}
...
///////////////
	...
	void daShyGuy::executeState_Dirt() {
		...
		if (this->timer < 120) {
			...
			if (this->direction == 1) {
				spawned = CreateActor(EN_KANITAMA, 0x2, pos, 0.5, 0);
				spawned->scale.x = 0.9;
				spawned->scale.y = 0.9;
				spawned->scale.z = 0.9;
				...
				spawned = CreateActor(EN_KANITAMA, 0x12, pos, -0.5, 0);
				spawned->scale.x = 0.9;
				spawned->scale.y = 0.9;
				spawned->scale.z = 0.9;
